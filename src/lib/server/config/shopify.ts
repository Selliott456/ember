...
function buildShopifyConfig(): ShopifyConfig {
	const storeDomain = required('SHOPIFY_STORE_DOMAIN', privateEnv.SHOPIFY_STORE_DOMAIN)
		.replace(/^https?:\/\//, '')
		.replace(/\/+$/, '');
	...
	const cartCookieName = privateEnv.SHOPIFY_CART_COOKIE_NAME?.trim() || 'cart_id';

	const cartCookieMaxAgeDays = Number(privateEnv.SHOPIFY_CART_COOKIE_MAX_AGE_DAYS ?? '30');
...
/**
 * Resolves Shopify config on first use. Safe to import during `vite build` / prerender
 * when env vars are not set; throws only when a code path actually calls Shopify.
 */
export function getShopifyConfig(): ShopifyConfig {
	if (!cached) {
		cached = buildShopifyConfig();
	}
	return cached;
}
